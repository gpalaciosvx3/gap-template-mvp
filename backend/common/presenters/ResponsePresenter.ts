...
export class ResponsePresenter {
  ...
  static badRequest(res: Response, error: { code: string; message: string; details?: unknown }): Response {
    return res.status(400).json(ResponsePresenter.fail(error));
  }

  static internalError(res: Response, error: { code: string; message: string; details?: unknown }): Response {
